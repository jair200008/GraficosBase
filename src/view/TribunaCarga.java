 ...
 *
 */
public class TribunaCarga extends JPanel implements MouseListener {
    ...
    private final double tipoPartido=Math.random() * 10;
    private int estadio;
    ...
    private void tribunasSuperiores() {
        ...
        if (tipo) {
            for (int i = 0; i < 3; i++) {
                ...
                for (int j = 0; j < 9; j++) {
                    bloqueo =Math.random() * 10;
                    
                    entero=(int)bloqueo+1;
                    if (entero%2 == 0) {
...
//REALIZARA LAS ACCIONES QUE EL USUARIO DESEA
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (e.getSource() == comida) {
            objetoCustom.llamarComida(2);
        }
        if (e.getSource() == atencion) {
            objetoCustom.llamarAtencion(2);
        }
    }
